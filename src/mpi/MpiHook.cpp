...
void MpiHook::injectHostLibrary(const SharedLibrary& hostLib,
    ...
    if (it == hostToContainerLibs.cend()) {
        ...
        libsarus::mount::validatedBindMount(hostLib.getPath(), containerLib, userIdentity, rootfsDir, 0);
    ...
    if(areCompatible.second == boost::none) {
        ...
        libsarus::mount::validatedBindMount(hostLib.getPath(), bestCandidateLib.getPath(), userIdentity, rootfsDir, 0);
...
void MpiHook::performBindMounts() const {
    ...
    for(const auto& mount : bindMounts) {
        ...
        libsarus::mount::validatedBindMount(mount, mount, userIdentity, rootfsDir, 0);
