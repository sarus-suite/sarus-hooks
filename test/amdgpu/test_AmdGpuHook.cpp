...
#include "libsarus/test/aux/unitTestMain.hpp"
...
namespace testaux = libsarus::test::aux;
...
void createOCIBundleConfigJSON(const boost::filesystem::path& bundleDir,
  ...
  auto doc = testaux::hook::createOCIBaseConfigJSON(bundleDir / "rootfs", idsOfUser);
...
TEST_GROUP(AMDGPUHookTestGroup) {
  std::tuple<uid_t, gid_t> idsOfUser = testaux::misc::getNonRootUserIds();
  boost::filesystem::path bundleDir = libsarus::PathRAII(libsarus::filesystem::makeUniquePathWithRandomSuffix(
                                                             boost::filesystem::current_path() / "AMDGPUHookTest"))
                                                        .getPath();
};
